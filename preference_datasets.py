...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None):
    ...
    if name == 'shp':
        ...
        data = get_base_uf(split, silent=silent, cache_dir=cache_dir)
    # elif name == 'llmaj_uf':
    #     data = get_llmaj_uf(split, silent=silent, cache_dir=cache_dir)
    else:
...
def get_slac_uf(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    # dataset = dataset.filter(lambda x: abs(float(x['score_chosen']) - float(x['score_rejected'])) >= 5.0)
    # print('done')
...
def get_base_uf(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    """Load the Ultrafeed dataset modified using SLAC.
    """
    print(f'Loading UF dataset ({split} split) from Huggingface...')
